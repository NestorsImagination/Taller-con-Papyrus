...
namespace UnityStandardAssets.Cameras
{
    ...
    {
		[SerializeField] private float m_UpOffset = 1; // How fast the rig will move to keep up with target's position
		[SerializeField] private float m_ManualRotationSpeed = 0.1f; // How fast the rig will move to keep up with target's position
        ...
        protected override void FollowTarget(float deltaTime)
        {
			...
			transform.position = Vector3.Lerp(transform.position, m_Target.position + new Vector3 (0, m_UpOffset, 0), deltaTime*m_MoveSpeed);
			...
			if ((Input.GetAxis ("Horizontal") != 0.0f)/* || (Input.GetAxis ("Vertical") > 0.0f)*/) {
				...
				transform.rotation = Quaternion.Lerp (transform.rotation, rollRotation, turnSpeed * m_CurrentTurnAmount * deltaTime);
			}

			float manualTurn = Input.GetAxis("MoveCamera");
			transform.Rotate(Vector3.up, manualTurn * m_ManualRotationSpeed * Time.deltaTime);
        }
    }
}